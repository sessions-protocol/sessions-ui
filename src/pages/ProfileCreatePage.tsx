...
const CreateProfileView: FunctionComponent = () => {
  ...
  async function onSubmitProfile() {
    try {
      ...
      // create profile
      const res = await sessionApi.createUserProfile(
        profileHandleRef.current.value,
        profilePictureRef.current?.value
      );
      console.log("profile created", res);
      // query profile
      const profile = await sessionApi.getProfileByHandle(
        profileHandleRef.current?.value
      );
      setProfileState({ profile: { ...profile, wallet: account! } });
  ...
  return (
    <>
      <div className="mb-6 grid gap-y-4">
        <FormControl>
          <FormLabel htmlFor="profile-input-handle">
            <span className="text-sm opacity-80">HANDLE</span>
          </FormLabel>
          <Input
            id="profile-input-handle"
            placeholder="your unique handle"
            ref={profileHandleRef}
            onInput={() => setName(profileHandleRef.current!.value)}
          />
        </FormControl>
        ...
        <FormControl>
          <FormLabel htmlFor="profile-input-picture">
            <span className="text-sm opacity-80">PROFILE PICTURE</span>
          </FormLabel>
          <Input
            id="profile-input-picture"
            placeholder="https://cc0.crypto.punks/lol.jpg"
      ...
      <Button
        ...
        disabled={creating}
        className={
          creating ? "cursor-not-allowed text-gray-400 bg-gray-50" : undefined
        }
      >
